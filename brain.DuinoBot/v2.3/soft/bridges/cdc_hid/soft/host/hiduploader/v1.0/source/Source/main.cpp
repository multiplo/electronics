...
void usage(void)
{
	fprintf(stderr, "Usage: hiduploader -mmcu=<MCU> [-v] -usb=<vid:pid> <file.hex>\n");
...
int main(int argc, char **argv)
{
	...
	printf("\nHID Uploader v1.0 (Based on code from Teensy Loader 2.0 - command line)\n");
	...
	while (ProgramDevice) {
        switch (ProgramState) {
            ...
            case ST_PROGRAMPAGE:
                ...
                memset(ControlBuf + 4, 0, sizeof(ControlBuf) - 4);
                r = teensy_write(ControlBuf, sizeof(ControlBuf), 0.05);
                if (!r) die("Error writing to device\n");
                printf_verbose(".");
            ...
            case ST_CLOSEDATAPAGE:
                ...
                memset(ControlBuf + 2, 0, sizeof(ControlBuf) - 2);
                r = teensy_write(ControlBuf, sizeof(ControlBuf), first_block ? 1.0 : 0.05);
                if (!r) die("Error writing to device\n");
                delay(0.01);
                // Next Block
                addr += block_size;
